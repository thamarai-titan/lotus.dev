  ...
  return (
    <main className="flex min-h-screen justify-center p-6 md:p-16 pb-24">
      <div className="w-full max-w-[640px] space-y-8">
        <BlogHeader
          title="Logs"
          description="Daily updates and dev logs of what I worked on, built, and learned today."
        />

        <section className="flex flex-col gap-2 font-sans antialiased">
          <h2 className="text-base text-muted-foreground font-medium">Daily Logs</h2>
          <Separator />

          <div className="flex flex-col gap-1 pt-1">
            {logs.length === 0 ? (
              <p className="text-muted-foreground text-sm py-4">No logs posted yet.</p>
            ) : (
              logs.map((log) => (
                <Link
                  key={log.slug}
